...
def page_widget_preview(request: Request, race_id: str):
    ...
    if not result.data:
        raise HTTPException(status_code=404, detail="Gara non trovata")
    race = result.data[0]
    widget_url = str(request.base_url) + f"widget/{race_id}"
    html = f"""<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .preview-name {{ font-size: 14px; color: rgba(255,255,255,0.7); }}
...
<body>
  ...
  <iframe src="{widget_url}" class="widget-iframe" title="Repliq Widget"></iframe>
</body>
</html>"""
